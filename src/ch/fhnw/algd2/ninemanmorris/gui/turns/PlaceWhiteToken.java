 ...
 */
public class PlaceWhiteToken extends TurnState {
    public PlaceWhiteToken(Controller controller) {
        super(controller);
    }

    @Override
    public Node validateToken(Token token) {
        for(Node node : controller.getModel().getWhiteStartTokens()) {
    ...
    public TurnState update() {
        if (controller.getModel().getBlackStartTokens().stream().filter(node -> node.getToken() != null).count() > 0) {
            return controller.getTurnManager().placeBlackTokens();
        } else {
